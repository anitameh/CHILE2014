...
if __name__ == "__main__":
	...
	# Load image data
	# 40 fields in total; create one process for each core
	field_path = sys.argv[1]

	if not os.path.isdir(field_path):
		print 'Error: Directory %s does not exist! Abort Program.'
		exit()
	field_num = field_path.split('/')[-2][-1]


	mask = field_path + 'image*.fits'
	fnamelist = glob.glob(mask)
	epochlist = []
	for fname in fnamelist:
		epoch = fname.split('_')[-2]
		epochlist.append(epoch)
	epochlist = set(epochlist)


	for ccd_idx in xrange(31):
		for ns in xrange(1):
			if ns == 1:
				ccd_ns = 'N'
			else:
			...
			for epoch in epochlist:
				...
				for shot_idx in xrange(4):
				


					fitpath = field_path + 'image_%s%s_Blind_%s_%s_%s_tu1680942.fits' %(ccd_ns, ccd_idx+1, field_num, shot_idx+1, epoch)
					if not os.path.exists(fitpath):
						continue
					
					fit= pyfits.open(fitpath)
					print 'processing %s' % fitpath

					# open output file
					cat_out = '/home/local/school/student19/scratch/catalogue/image_%s_%s%s_%s.cat' % (field_num, ccd_ns, ccd_idx+1, epoch)
					fp = open(cat_out, 'a+') # Opens a file for both appending and reading.										Cat.append(Dat)
